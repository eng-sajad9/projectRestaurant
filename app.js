          ...
          function applyMaintenanceState(state){
            try{
              ...
              try{
                ...
                if(overlayMsg) overlayMsg.textContent = message;تح
                if(txtArea && document.activeElement !== txtArea) txtArea.value = message;
              }catch(e){}
              if(toggleBtn) {
                toggleBtn.textContent = enabled ? 'إيقاف وضع الصيانة' : 'تفعيل وضع الصيانة';
  ...
  `;
      ...
      function loadEmployeesTable() {
        ...
        employees.forEach((emp, idx) => {
          ...
          detailsBtn.style.background = "#17a2b8";
      ...
      function renderStatsDOM(counts, allowedEmps, canViewSalary) {
        ...
        if (statsDiv) {
          statsDiv.innerHTML = `
            <p><strong>🏆 أكثر موظف شفت:</strong> ${
              bestShift ? bestShift[0] + ` (${bestShift[1]["شفت"]})` : "—"
            }</p>
            <p><strong>⭐ أكثر موظف نص:</strong> ${
              bestHalf ? bestHalf[0] + ` (${bestHalf[1]["نص"]})` : "—"
            }</p>
            <p><strong>❌ أكثر موظف غياب:</strong> ${
              mostX ? mostX[0] + ` (${mostX[1]["❌"]})` : "—"
            }</p>`;
        }
              ...
              db.ref("users/" + username + "/logViewType")
                ...
                .then(() => {
                  logActivity(
                    ...
                    `تم تغيير نوع الصلاحية للمستخدم: ${username} إلى: ${
                      type === "all" ? "كل السجل" : "سجل المستخدم فقط"
                    }`
                  );
                  updateSidebarVisibility();
                });
